...
TensorFlow does not officially support multiple GPU profiling yet. You can install CUDA® Toolkit 10.2 or later to profile multiple GPUs. As TensorFlow supports CUDA® Toolkit versions only up to 10.1 , create symbolic links to `libcudart.so.10.1` and `libcupti.so.10.1`.
